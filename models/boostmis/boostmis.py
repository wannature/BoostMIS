...
from .boostmis_utils import consistency_loss, Get_Scalar
...
class boostmis:
    def __init__(self, net_builder, num_classes, ema_m, T, a,b, N_a, lambda_u, \
                 hard_label=True, t_fn=None, p_fn=None, it=0, num_eval_iter=1000, tb_log=None, logger=None):

        super(boostmis, self).__init__()
        ...
        self.p_fn = Get_Scalar(float(a)+float(b))  # confidence cutoff function
    ...
    def train(self, args, logger=None):
        ...
        self.loader_dict['train_lb'].dataset.num_label= 40
        self.loader_dict['train_ulb'].dataset.num_label= 40
        for (_, x_lb, y_lb), (x_ulb_idx, x_ulb_w, x_ulb_s) in zip(self.loader_dict['train_lb'],
            ...
            with amp_cm():
                ...
                unsup_loss, mask, select, pseudo_lb, p_model = consistency_loss(logits_x_ulb_s,
                                                                                ...
                                                                                p_target,
                                                                                p_model,
                                                                                'ce', T, args.a,args.b, args.p_cutoff,N_a=100, K=2*args.K,  
                                                                                use_hard_labels=args.hard_label,
                                                                                use_DA=args.use_DA)

                if 0[select == 1].nelement() != 0:
