...
from models.boostmis.boostmis import boostmis
from datasets.ssl_dataset import SSL_Dataset, ImageNetLoader
from datasets.data_utils import get_data_loader
from models.boostmis.al import al_selection
def get_class_indices(data_list):
    data_class_0=[]
    data_class_1=[]
    for i in range(len(data_list)):
        if data_list[i][1]==0:
            data_class_0.append(i)
        if data_list[i][1]==1:
            data_class_1.append(i)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # SET boostmis: class boostmis in models.boostmis
    ...
    ## set SGD and cosine lr on boostmis
    ...
    if args.dataset.lower() != "imagenet":
        train_dset = SSL_Dataset(args, alg='boostmis', name=args.dataset, train=True,
                                num_classes=args.num_classes, data_dir=args.data_dir)
        lb_dset, ulb_dset = train_dset.get_ssl_dset(args.num_labels)
        _eval_dset = SSL_Dataset(args, alg='boostmis', name=args.dataset, train=False,
                                num_classes=args.num_classes, data_dir=args.data_dir)
    ...
    data_class_0,data_class_1= get_class_indices(lb_dset.imgs)
    current_indices=random.sample(data_class_0, 250)+random.sample(data_class_1, 250)
    random.shuffle(current_indices)
    unlabeled_indices= list(set(data_class_0+data_class_1)-set(current_indices))
    random.shuffle(unlabeled_indices)
    loader_dict['train_lb'] = get_data_loader(dset_dict['train_lb'],
                                              args.batch_size,
                                              data_sampler=SubsetRandomSampler(current_indices),
    ...
    loader_dict['train_ulb'] = get_data_loader(dset_dict['train_ulb'],
                                               ...
                                               data_sampler=SubsetRandomSampler(unlabeled_indices),
                                               num_iters=args.num_train_iter,
    ...
    ## set DataLoader and ulb_dset on boostmis
    ...
    # START TRAINING of boostmis
    trainer = model.train
    for epoch in range(args.epoch):
        trainer(args, logger=logger)
        al_indices=al_selection(args, model,loader_dict['train_ulb'] )
        args.N_a=len(al_indices)
        current_indices=current_indices+al_indices
        loader_dict['train_lb'] = get_data_loader(dset_dict['train_lb'],
                                            args.batch_size,
                                            data_sampler=SubsetRandomSampler(current_indices),
                                            num_iters=args.num_train_iter,
                                            num_workers=args.num_workers,
                                            distributed=args.distributed)
        unlabeled_indices=unlabeled_indices-al_indices
        loader_dict['train_ulb'] = get_data_loader(dset_dict['train_ulb'],
                                            args.batch_size * args.uratio,
                                            data_sampler=SubsetRandomSampler(unlabeled_indices),
                                            num_iters=args.num_train_iter,
...
if __name__ == "__main__":
    ...
    parser.add_argument('-sn', '--save_name', type=str, default='boostmis')
    parser.add_argument('--resume', action='store_true')
    parser.add_argument('--load_path', type=str, default=None)
    parser.add_argument('-o', '--overwrite', action='store_true')

    '''
    Training Configuration of boostmis
    '''
